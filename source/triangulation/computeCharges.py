...
def assignChargesToNewMesh(new_vertices, old_vertices, old_charges, seeder_opts):
    ...
    if seeder_opts["feature_interpolation"]:
        ...
        # Square the distances (as in the original pyflann: NEEDS reference https://stackoverflow.com/questions/16330194/flann-in-matlab-returns-different-distance-from-my-own-calculation)
        dists = np.square(dists)
        # np.save(seeder_opts['ply_chain_dir']+'test.npy', {'dists':dists,
        #                                                   'result':result})
        ...
        # print(new_charges[17], old_charges[result[17]],dists[17])
    else:
        # Assign k old vertices to each new vertex.
        kdt = KDTree(dataset)
...
def assignAtomNamesToNewMesh(new_vertices, old_vertices, old_names):
    dataset = old_vertices
    testset = new_vertices
    kdt = KDTree(dataset)
    dists, result = kdt.query(testset)
    new_names = np.array(old_names)[result]
    return new_names
