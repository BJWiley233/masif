...
def write_pdb2(inds, what, xs, ys, zs, chain='C', file="ml_out.pdb"):
    ...
    fpdb = open("pdbs_nowaterSim%d/%s"%(Sim,file), 'wt')
...
def get_frame_start(start, s=1.25, min_dist = 4.0, min_res=0.3):
    ...
    protein_coords_traj_use = coords[nogasnowater] # without gas and without water
    ...
    scores, points, have_positives = topk_with_radius(results3, pos_embedding, k=4, radius=4, threshold=min_res)
...
frames = np.arange(0,850, 50)
starting_points = []
for f in frames:
    print(f, flush=True)
    if f == 0:
        kept_positions, results3 = get_frame_start(f, s=1.5, min_dist=4., min_res=0.55)
    else:
        kept_positions, results3 = get_frame_start(f, s=1.5, min_dist=4., min_res=0.55)
...
for predicted_frame_start, sps in zip(frames, starting_points):
    ...
    for sp in sps:
        ...
        for start in range(start+0,start+end-1):
            ...
            biased_string=str(start)+ '_Sim%d_frame'%Sim + str(step) + '_of_' + str((end)) + '_biased.pdb'
            write_pdb2(torch.arange(0,len(pos_embedding)),results_biased,xyz2[:,0],xyz2[:,1],xyz2[:,2], file=biased_string)
            ...
            print('mol new pdbs_nowaterSim%d/'%(Sim) + biased_string, flush=True)
            print('mol addfile pdbs_nowaterSim%d/translated_frame%d_Sim%d.pdb'%(Sim,start,Sim), flush=True)
            step+=1
