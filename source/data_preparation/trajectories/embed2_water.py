...
def embed_event_neg(path, gas_resid, gas_idx, in_row, start, end, protein_cofactors, global_to_local, direction):
    ...
    for idx, c in enumerate(range_):
        ...
        min_dist = 2.75
        min_dists = [2.25, 2.5, 2.75, 3.0, 3.1]
        max_dist = 4.
        
        for w in range(5):
            good_2_cont = True
            min_dist = min_dists[w]
            tried=0
            while True:
                tried+=1
                if tried%5000==0:
                ...
                if tried < 3000:
                    if tried % 1000 == 0:
                        print('trying negative [2., 7.]:', tried, flush = True)
                    step = torch.empty(1, device=atom_coords.device).uniform_(2., 7.)
                elif tried < 8000:
                    if tried % 1000 == 0:
                        print('trying negative [1., 7.]:', tried, flush = True)
                    step = torch.empty(1, device=atom_coords.device).uniform_(1., 7.)
                elif tried <= 15000:
                    if tried % 1000 == 0:
                        print('trying negative [1., 8.]:', tried, flush = True)
                    step = torch.empty(1, device=atom_coords.device).uniform_(1., 8.)
                elif tried > 15000:
                    good_2_cont = False
                    break
            ...
            if good_2_cont:
                ...
                print("\tneg sequence %d (w=%d) of %d; frame %d" % (idx, w,(end-start), c), "len indices:", len(local_inside_indices), flush=True)
