...
for ppi_pair_id in ppi_pair_list:
    ...
    for pid in pids:
        print(pid)
        # print("Brian")
        input_feat[pid], rho[pid], theta[pid], mask[pid], neigh_indices[pid], iface_labels[pid], verts[pid] = read_data_from_surface(ply_file[pid], params, pid)
        print(len(verts[pid]))
    ...
    for pid in pids: 
        ...
        # Save x, y, z
        print("####################################"+my_precomp_dir+pid, 'verts[pid][:,0]', verts[pid].shape)
