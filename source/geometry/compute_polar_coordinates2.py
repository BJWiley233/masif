...
def compute_polar_coordinates(mesh, do_fast=True, radius=12, max_vertices=200, params=None, pid=None):
    """
    ...
    # bidirectional graph with three nodes each face
    ...
    # np.save(params["masif_precomputation_dir"]+'/{}_wedges.npy'.format(pid), wedges)
    ...
    #     'idx':idx
    # }
    # np.save('{}_save.npy'.format(pid), save)

    if os.path.exists(params["masif_precomputation_dir"]+'/{}_theta.npy'.format(pid)):
        print("LOADING THETA FROM FILE")
        print(params["masif_precomputation_dir"]+'/{}_theta.npy'.format(pid))
        theta = np.load(params["masif_precomputation_dir"]+'/{}_theta.npy'.format(pid))
    else:
        print('MDS start')
        if do_fast:
            theta = compute_theta_all_fast(D, vertices, faces, normals, idx, radius, mesh, params, pid)
        else:
            theta = compute_theta_all(D, vertices, faces, normals, idx, radius)

        # np.save(params["masif_precomputation_dir"]+'/{}_theta.npy'.format(pid), theta)
        
    sys.stdout.flush()
    # Output a few patches for debugging purposes.
    # extract a patch
    # print('extract subpatches')
    # for i in [0,100,500,1000,1500,2000,4784, 5626, 6242, 6595, 7037]:
    # for i in [0]:
    #     neigh = D[i].nonzero()
    #     ii = np.where(D[i][neigh] < radius)[1]
    #     # np.save('D.npy',D)
    #     if radius >=6:
    #         ii2 = np.where(((D[i].toarray()[0]>0) & (D[i].toarray()[0] < radius/2)))
    #     else:
    #         ii2 = np.where(((D[i].toarray()[0]>0) & (D[i].toarray()[0] < 2.75)))
    #     print("ii2", ii2)
    #     neigh_i = neigh[1][ii]
    #     #    sub_D = D[np.where(D[i][neigh] < radius)[1]]
    #     # np.array(subverts), np.array(subn), np.array(subf), np.array(subchg) 
    #     subv, subn, subf, subchg, subi = extract_patch(mesh, neigh_i, i, ii2)
    #     # Output the patch's rho and theta coords
    #     # output_patch_coords(subv, subf, subn, i, neigh_i, theta, rho):
    #     output_patch_coords_wcharge(subv, subf, subchg, subn, subi, i, neigh_i, theta[i], D[i, :])
    #     # output_patch_coords(mesh, subn, i, neigh_i, theta[i], D[i, :])
...
def compute_thetas(plane, vix, verts, faces, normal, neighbors, idx, mesh):
    """
    ...
    normals. 
    Args: 
        plane: the 2D plane of the vertices in the patch as computed by multidimensional scaling (MDS)
    ...
    try:
        ...
        # think there is a hole in the mesh
        subv, subn, subf, subchg, subi = extract_patch(mesh, neighbors, vix)
        output_patch_coords_wcharge(subv, subf, subchg, subn, subi, vix, neighbors)
        print('error: saving v{}.ply'.format(vix))
        # set_trace()
     
    # Compute the normal for tt by averaging over the vertex normals
...
def extract_patch(mesh, neigh, cv, ii2):
    """ 
    Extract a patch from the mesh.
        neigh: the neighboring vertices.
        cv: the vertex center (make the charge super negative)
    """
    n = len(mesh.vertices)
    subverts = mesh.vertices[neigh]
    chg = mesh.get_attribute('vertex_charge').copy()
    chg = np.zeros(len(chg))
    
    # print(chg[ii2], neigh)
    chg[ii2] = 30
    chg[cv] = -30
    subchg = chg[neigh]
    subi = mesh.get_attribute('vertex_iface')[neigh]
    ...
    for i in range(len(neigh)):
        m[neigh[i]] = i
    print("DEBUG", len(neigh), m[m>0].sum())
    ...
    # subfaces = subf
    return np.array(subverts), np.array(subn), np.array(subf), np.array(subchg), np.array(subi) 
...
def output_patch_coords(subv, subf, subn, i, neigh_i, theta, rho): 
    ...
    if not mesh.has_attribute('vertex_charge'):
...
def output_patch_coords_wcharge(subv, subf, subc, subn, subi, i, neigh_i, theta=None, rho=None): 
    """ 
        For debugging purposes, save a patch to visualize it.
    """ 
    print("\t1")
    ...
    mesh.set_attribute('vertex_charge', subc)
    mesh.add_attribute('vertex_iface')
    mesh.set_attribute('vertex_iface', subi)
    print("\t3")
    if rho is not None:
        rho = np.array([rho[0,ix] for ix in range(rho.shape[1]) if ix in neigh_i])
        mesh.add_attribute('rho')
...
def compute_theta_all(D, vertices, faces, normals, idx, radius):
    mymds = MDS(n_components=2, n_init=1, max_iter=50, dissimilarity='precomputed', n_jobs=12)
    all_theta = []
...
def compute_theta_all_fast(D, vertices, faces, normals, idx, radius, mesh, params=None, pid=None):
    """
    ...
    for i in range(D.shape[0]):
        ...
        if radius >= 6:
            # print('radius', radius)
            ii = np.where(D[i][neigh] < (radius*0.5))[1]
        else:
            ii = np.where(D[i][neigh] < 2.75)[1]
        neigh_i = neigh[1][ii]
        pair_dist_i = D[neigh_i,:][:,neigh_i]
        pair_dist_i = pair_dist_i.todense()
        # Plane_i: the 2D plane for all neighbors of i
        ...
        try:
        # Compute the angles on the plane.
            theta = compute_thetas(plane_i, i, vertices, faces, normals, neigh_i, idx, mesh)
        
        except Exception as e: 
            print("Brian Error:",e, i)
            # params["masif_precomputation_dir"]+'/{}_theta.npy'.format(pid)
            np.save(params["masif_precomputation_dir"]+"/%s_%d_test.npy"%(pid,i),
                    {
                        'i':i,
                        'plane_i':plane_i,
                        'vertices':vertices,
                        'faces':faces, 'normals':normals, 'neigh_i':neigh_i, 'idx':idx
                    })
