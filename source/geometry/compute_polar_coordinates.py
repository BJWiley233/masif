...
def compute_polar_coordinates(mesh, do_fast=True, radius=12, max_vertices=200, params=None, pid=None):
    ...
    if os.path.exists('{}_theta.npy'.format(pid)):
        ...
        theta = np.load('{}_theta.npy'.format(pid))
    else:
        print('MDS start')
        if do_fast:
            theta = compute_theta_all_fast(D, vertices, faces, normals, idx, radius, mesh)
        else:
        ...
        np.save('{}_theta.npy'.format(pid), theta)
    sys.stdout.flush()
...
def compute_theta_all_fast(D, vertices, faces, normals, idx, radius, mesh):
    """
