...
class MaSIF_ligand2:
    ...
    def inference(
        ...
        for k in range(self.n_rotations):
            ...
            rho_coords2 = tf.identity(rho_coords_)
        ...
        with tf.get_default_graph().as_default() as g:
            ...
            for pr in range(1):
                ...
                for i in range(self.n_feat):
                    # '*_*_0:0' = si, '*_*_1:0' = ddc, '*_*_2:0' = hbond, ...
                    self.mu_rho.append(
                ...
                self.keep_prob = tf.placeholder(tf.float32) #  learning_obj.keep_prob: 1.0
                self.name = tf.placeholder(tf.string)
                self.ligand = tf.placeholder(tf.string)
                # print('self.name', self.name, 'self.ligand', self.ligand)
                ...
                if self.print:
                    print("printint")
                    np.save("_test2.npy", self.input_feat.eval())
                self.mask = tf.placeholder(tf.float32)  # batch_size, n_vertices, 1
                self.labels = tf.placeholder(tf.float32) # learning_obj.labels: pocket_labels,
                self.global_desc_1 = []
                self.rho_coords2 = []
                b_conv = []
                for i in range(self.n_feat):
                ...
                self.W_conv = []
                for i in range(5):
                    init=glorot_uniform((80, 80), seed=(i+1)*100)
                    print(init.sum())
                    self.W_conv.append(
                            tf.Variable(init,
                ...
                # print([type(i) for i in self.W_conv])
                ...
                for i in range(self.n_feat):
                    ...
                    print("my_input_feat",type(my_input_feat))
                    ...
                    self.global_desc_1.append(conv_feat)  # batch_size, n_gauss*1
                
                
                print("self.global_desc_1.shape 1", len(self.global_desc_1))
                ...
                print("self.global_desc_1.shape 2", self.global_desc_1.shape)
                ...
                print("self.global_desc_1.shape 3", self.global_desc_1.shape)
                ...
                self.global_desc_2=self.global_desc_1
                print("self.global_desc_1.shape 4", self.global_desc_1.shape)
                ...
                print("self.global_desc_1.shape 5", self.global_desc_1.shape)
                self.global_desc_1 = tf.reshape(self.global_desc_1, [1, -1])
                print("self.global_desc_1.shape 6", self.global_desc_1.shape)
                self.global_desc_1 = tf.nn.dropout(self.global_desc_1, self.keep_prob)
                print("self.global_desc_1.shape 7", self.global_desc_1.shape)
                self.global_desc_1 = tf.keras.layers.Dense(
                    # self.global_desc_1, 
                    64, activation=tf.nn.relu
                )(self.global_desc_1)
                
                print("self.global_desc_1.shape 8", self.global_desc_1.shape)
                # self.global_desc_2=self.global_desc_1
                self.logits = tf.keras.layers.Dense(
                    # self.global_desc_1, 
                    self.n_ligands, activation=tf.identity
                )(self.global_desc_1)
                print("self.logits.shape 1", self.logits.shape)
                ...
                print('self.data_loss',self.data_loss)
