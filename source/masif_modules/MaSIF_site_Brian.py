...
class MaSIF_site:
    ...
    def inference(
        ...
        return conv_feat, mu_rho, sigma_rho, mu_theta, sigma_theta
    ...
    def __init__(
        ...
        with tf.Graph().as_default() as g:
            ...
            with tf.device(idx_gpu):
                ...
                self.spos_idx = tf.placeholder(tf.int32)  # batch_size/2
                ...
                for i in range(self.n_feat):
                    ...
                    self.sigma_theta2 = []
                    self.rc2 = rho_coords
                    conv_feat, mu_rho, sigma_rho, mu_theta, sigma_theta = self.inference(
                            my_input_feat,
                            rho_coords,
                            theta_coords,
                            mask,
                    ...
                    self.W_conv.append(W_conv)
                    # self.global_desc2.append(
                    #     tf.identity(conv_feat)
                    # ) 
                    self.global_desc.append(
                        conv_feat
                    )  # batch_size, n_gauss*1
                    self.mu_rho2.append(mu_rho)
                    self.sigma_rho2.append(sigma_rho)
                    self.mu_theta2.append(mu_theta)
                    self.sigma_theta2.append(sigma_theta)
                ...
                # self.global_desc2 = tf.stack(self.global_desc2, axis=1)
                self.global_desc = tf.reshape(
                    self.global_desc, [-1, self.n_thetas * self.n_rhos * self.n_feat]
                )
                # self.global_desc2 = tf.reshape(
                #     self.global_desc2, [-1, self.n_thetas * self.n_rhos * self.n_feat]
                # )
                ...
                self.global_desc2=self.global_desc
                # self.global_desc2 = tf.contrib.layers.fully_connected(
                #     self.global_desc2,
                #     self.n_thetas * self.n_rhos,
                #     activation_fn=tf.nn.relu,
                # )
                ...
                self.global_desc3=self.global_desc
                # self.global_desc2=self.global_desc
                ...
                # see if you can do output mlp to new tf2
                # return self.d2(x) from call()
                # self.d2 = keras.layers.Dense(self.global_desc,
                #         self.n_labels, activation='softmax')
                # self.logits = tf.expand_dims(self.logits, axis=0)
                # https://github.com/tensorflow/tensorflow/issues/37683
                self.eval_labels = tf.concat(
                    [
                        tf.gather(self.labels, self.spos_idx),
                ...
                self.eval_logits = tf.concat(
                    [
                        tf.gather(self.logits, self.spos_idx),
                        tf.gather(self.logits, self.pos_idx),
                        tf.gather(self.logits, self.neg_idx),
                    ],
                    axis=0,
                )
                self.data_loss = tf.nn.softmax_cross_entropy_with_logits_v2(
            ...
            # return initial_coords, self.mu_rho
