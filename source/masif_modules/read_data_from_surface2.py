...
from geometry.compute_polar_coordinates2 import compute_polar_coordinates
from input_output.save_ply import save_ply

from sklearn import metrics


def read_data_from_surface(ply_fn, params, protein=True, pid=None):
    """
    ...
    print('params[max_shape_size]', params['max_shape_size'])
    ...
    # H[np.isnan(H)] = 0.0
    print("vertex_mean_curvature", H.min(), H.max(), H.mean())
    # np.save(pid+'_mean_curvature', rho)

    mesh.add_attribute("vertex_gaussian_curvature")
    K = mesh.get_attribute("vertex_gaussian_curvature")
    # K[np.isnan(K)] = 0.0
    print("vertex_gaussian_curvature", K.min(), K.max(), K.mean())
    
    elem = np.square(H) - K
    print("elem", elem.min(), elem.max(), elem.sum())
    # In some cases this equation is less than zero, likely due to the method that computes the mean and gaussian curvature.
    # set to an epsilon.
    elem[elem<0] = 1e-8
    elem[np.isnan(elem)] = 1e-8
    k1 = H + np.sqrt(elem)
    k2 = H - np.sqrt(elem)
