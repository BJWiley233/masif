...
def iface_color2(iface):
    # max value is 2, min values is 0, 2 for also near cofactor, 1 for near gas
    hp = iface.copy()
    hp = hp - 1
    print("Sum hp", sum(hp))
    mycolor = charge_color(-hp)
...
def load_ply(
    ...
    try:
        ...
        print("len(color_array)", len(color_array))
    except:
    ...
    if (
        ...
        for tri in faces:
            ...
            obj.extend([VERTEX, (vert3[0]), (vert3[1]), (vert3[2])])
            obj.append(END)
        name = "hphobic_" + os.path.basename(filename)
        print("hphobic_", name)
        cmd.load_cgo(obj, name, 1.0)
        obj = []
        group_names = group_names + " " + name
    ...
    if (
        ...
        print("iface_color2", iface_color2)
        print("BRIAN", iface_color2)
        color_array_surf = iface_color2(iface)
        for tri in faces:
            vert1 = verts[int(tri[0])]
            vert2 = verts[int(tri[1])]
            vert3 = verts[int(tri[2])]
        ...
        np.save("/data/pompei/bw973/Oxygenases/masif/source/test/test.npy", obj)
        # cmd.set("cgo_transparency", 2.0 , name)
        # cmd.load_cgo(obj, name, 1.0)
        cmd.load_cgo(obj, "BRIAN2", 1.0)
        cmd.set("cgo_transparency", 0.25 , "BRIAN2")
        cmd.set("transparency_mode", 3 , "BRIAN2")
